...
export function App() {
  ...
  return <ScreenTransition screenKey={recordsTab ? `records-${recordsTab}` : 'menu'} animated={animateScreenTransition}><main className="app-shell"><Card className={`menu-card ${recordsTab ? 'menu-with-records' : ''}`} aria-labelledby="game-title">
    {recordsTab ? <h1 className="log-title" id="game-title">Captain’s Log</h1> : <><div className="menu-brand"><img className="game-logo" src="/assets/png/retina/ui/menu/title_pirate_battle.png" alt="" /><Button type="button" sound="uiOpen" variant="secondary" className="mobile-only fullscreen-button" aria-label={isFullscreen ? 'Exit Fullscreen' : 'Fullscreen'} aria-pressed={isFullscreen} onClick={() => { void toggleFullscreen(); }}><span className="fullscreen-glyph" aria-hidden="true">{isFullscreen ? '×' : '⛶'}</span><span>{isFullscreen ? 'Exit' : 'Fullscreen'}</span></Button></div><h1 className="visually-hidden" id="game-title">Pirate Battle</h1><p className="eyebrow">Set sail. Take command.</p></>}
