...
type SoundButtonProps = ComponentProps<typeof BaseButton> & { readonly sound?: SoundName | false };

export function Button({ className, variant, children, sound = 'uiClick', onClick, onPointerEnter, ...props }: SoundButtonProps) {
  return <BaseButton {...props} onClick={(event) => { if (sound) soundManager.play(sound, { volume: 0.42 }); onClick?.(event); }} onPointerEnter={(event) => { if (!props.disabled && matchMedia('(hover: hover)').matches) soundManager.play('uiHover', { volume: 0.2, throttleMs: 80 }); onPointerEnter?.(event); }} asChild variant={variant} className={cn('pirate-button', (variant === 'secondary' || variant === 'link') && 'pirate-button-secondary', className)}>
    <m.button whileHover={{ scale: 1.015 }} whileTap={{ scale: 0.97 }} transition={{ duration: 0.14, ease: 'easeOut' }}>{children}</m.button>
  </BaseButton>;
}

export function RoundButton({ icon, className, sound = 'uiClick', onClick, onPointerEnter, ...props }: SoundButtonProps & { icon: SpriteIcon }) {
  return <BaseButton {...props} onClick={(event) => { if (sound) soundManager.play(sound, { volume: 0.42 }); onClick?.(event); }} onPointerEnter={(event) => { if (!props.disabled && matchMedia('(hover: hover)').matches) soundManager.play('uiHover', { volume: 0.2, throttleMs: 80 }); onPointerEnter?.(event); }} asChild type="button" className={cn('pirate-round', className)}><m.button whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.9 }} transition={{ duration: 0.12, ease: 'easeOut' }}><Icon name={icon} /></m.button></BaseButton>;
}
